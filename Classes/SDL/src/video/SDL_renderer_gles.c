...
static int GLES_RenderCopy(SDL_Renderer * renderer, SDL_Texture * texture,
                           ...
                           const SDL_Rect * dstrect, textureRenderFlags renderFlags);
...
*/
	...
	// KP: always keep these default settings active, since changing render state is costly
...
GLES_CreateTexture(SDL_Renderer * renderer, SDL_Texture * texture)
{
	...
	{
		// KP: changed to power of 2, for faster texture updates
        data->pitch = power_of_2(texture->w) * SDL_BYTESPERPIXEL(texture->format);
        data->pixels = SDL_malloc(power_of_2(texture->h) * data->pitch);
        if (!data->pixels) {
            SDL_OutOfMemory();
            SDL_free(data);
            return -1;
...
/*	
	...
	data->glEnableClientState(GL_TEXTURE_COORD_ARRAY);
	lastTexture = -1;
// KP: the second time around, GLES_SetBlendMode() does nothing, so this will incorrectly always disable blending...	
//    data->glDisable(GL_BLEND);
*/
	renderQueueAddFill(vertices, (GLfloat) renderer->r * inv255f, (GLfloat) renderer->g * inv255f, (GLfloat) renderer->b * inv255f, (GLfloat) renderer->a * inv255f);
...
GLES_RenderCopy(SDL_Renderer * renderer, SDL_Texture * texture,
                const SDL_Rect * srcrect, const SDL_Rect * dstrect, textureRenderFlags renderFlags)
{
...
//	}
    ...
    if (texturedata->dirty.list) 
	{
		...
		// KP: this is already done in the line above
        //data->glBindTexture(texturedata->type, texturedata->texture);

// KP: recreating the whole texture is faster than doing a glTexSubImage2D call...		
#if 0
        for (dirty = texturedata->dirty.list; dirty; dirty = dirty->next) 
		{
			...
			
			{
				...
				temp_ptr = (void *) ((Uint8 *) texturedata->pixels + rect->y * pitch +
						  rect->x * bpp);
				for (i = 0; i < rect->h; i++) 
				{
					SDL_memcpy(temp_ptr, pixels, rect->w * bpp);
					temp_ptr += pitch; //rect->w * bpp;
					pixels += pitch;
				}
		...
		data->glBindTexture(texturedata->type, texturedata->texture);
		data->glTexImage2D(texturedata->type, 0, GL_RGBA, power_of_2(texture->w), power_of_2(texture->h), 0, texturedata->format, texturedata->formattype, texturedata->pixels);
...
//        data->glDisableClientState(GL_VERTEX_ARRAY);
		...
		renderQueueAddTexture(vertices, texCoords, texturedata->texture,0xFFFFFFFF,1.0f);
    }
...
//        glDeleteTextures(1, &data->texture);
		renderQueueDeleteTexture(data->texture);
    }
    if (data->pixels) {
