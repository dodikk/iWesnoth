/* $Id: ai.cpp 34673 2009-04-10 11:13:24Z dragonking $ */
/*
...
protected:
	...
	bool do_recruitment() {
		const std::set<shared_string>& options = current_team().recruits();
		if (!options.empty()) {
			const int choice = (rand()%options.size());
		        std::set<shared_string>::const_iterator i = options.begin();
...
std::vector<shared_string> get_available_ais()
{
    std::vector<shared_string> ais;
...
bool ai_interface::recruit(const std::string& unit_name, location loc)
{
	...
	const std::set<shared_string>::const_iterator i = recruits.find(unit_name);
	if(i == recruits.end()) {
		return false;
	}
...
void ai::analyze_potential_recruit_combat()
{
	...
	const std::set<shared_string>& recruits = current_team().recruits();
	std::set<shared_string>::const_iterator i;
...
void ai::analyze_potential_recruit_movements()
{
	...
	for(std::set<shared_string>::const_iterator i = recruits.begin(); i != recruits.end(); ++i) {
		const unit_type_data::unit_type_map::const_iterator info = unit_type_data::types().find_unit_type(*i);
	...
	for(std::map<shared_string,int>::iterator j = unit_movement_scores_.begin();
...
bool ai::do_recruitment()
{
	...
	std::vector<shared_string> options = current_team().recruitment_pattern();
