/* $Id: builder.cpp 32985 2009-02-21 22:39:05Z ilor $ */
/*
...
{
	rule_image_variantlist::const_iterator tod_variant = itor->second->variants.find(tod);
	...
	building_rules_()
{
	...
	// check if we are running the minimal startup config, and do not load this 
	if (!level.empty())
	{
		std::string filename = get_cache_dir() + "/terrain";
		...
		{
			parse_config(cfg);		// KP: takes about 5s on PC, forever on device
			parse_config(level);
...
void terrain_builder::replace_token(shared_string &s, const std::string &token, const std::string &replacement)
{
	size_t pos;
	...
	std::string temp = s;
	while((pos = temp.find(token)) != std::string::npos) {
		temp.replace(pos, token.size(), replacement);
	}
	s = temp;
}
...
void terrain_builder::saveCache(FILE *file) const
{
	// typedef std::multimap<int, building_rule> building_ruleset;
	
	short numRules = building_rules_.size();
	fwrite(&numRules, sizeof(numRules), 1, file);
	
	building_ruleset::const_iterator rule;
	for(rule = building_rules_.begin(); rule != building_rules_.end(); ++rule) 
	{
		int iFirst = rule->first;
		fwrite(&iFirst, sizeof(iFirst), 1, file);
		rule->second.saveCache(file);
	}
}

void terrain_builder::loadCache(std::string filename)
{
...
void terrain_builder::saveCache(std::string filename) const
{
	unsigned long startTicks = SDL_GetTicks();
...
void terrain_builder::loadCache(MEMFILE *file, char *loadBuffer)
{
	short numRules;
	mread(&numRules, sizeof(numRules), 1, file);
	//building_rules_.rehash(numRules);		// boost::unordered_map optimization
	for (int i=0; i < numRules; i++)
	{
		int iFirst;
		mread(&iFirst, sizeof(iFirst), 1, file);
		
		building_rule rule;
		//rule.loadCache(file, loadBuffer);
		
		building_ruleset::iterator it;
		it = building_rules_.insert(std::pair<int, building_rule>(iFirst, rule));
