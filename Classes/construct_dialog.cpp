/* $Id: construct_dialog.cpp 31859 2009-01-01 10:28:26Z mordante $ */
/*
...
const size_t dialog::top_padding = 3;//font::relative_size(10);
	const size_t dialog::bottom_padding = 3;//font::relative_size(10);
...
void dialog::set_menu_items(const std::vector<shared_string> &menu_items)
{
	if(menu_ == empty_menu) {
		set_menu(menu_items);
	} else {
...
//	surface const scr = screen.getSurface();
	...
	std::string msg = message_->get_text();
	if(!msg.empty() && msg != " ") {
	...
	dim.interior.x = std::max<int>(0,dim.x >= 0 ? dim.x : screen.getx()/2 - (dim.interior.w + left_preview_pane_width + right_preview_pane_width)/2);
	dim.interior.y = std::max<int>(0,dim.y >= 0 ? dim.y : screen.gety()/2 - (dim.interior.h + above_preview_pane_height)/2);

//	std::cerr << "above_preview_pane_height: " << above_preview_pane_height << "; "
//		<< "dim.interior.y: " << screen.gety()/2 << " - " << (dim.interior.h + above_preview_pane_height)/2 << " = "
//		<< dim.interior.y << "; " << "dim.interior.h: " << dim.interior.h << "\n";
	...
	if(dim.x + dim.interior.w > screen.getx()) {
		dim.x = screen.getx() - dim.interior.w;
	...
	if(dim.y + dim.interior.h + frame_bottom_pad > screen.gety()) {
		dim.y = std::max<int>(frame_top_pad, screen.gety() - dim.interior.h - frame_bottom_pad);
	...
	const int max_height = screen.gety() - dim.interior.y - frame_bottom_pad;
	if(dim.interior.h > max_height) {
		 ...
		 */
		// KP: forget it... make it a fullscreen dialog then
		dim.interior.y = 0;
		dim.y = frame_top_pad + above_preview_pane_height;
		dim.interior.h = screen.gety() - frame_bottom_pad;
...
int dialog::process(dialog_process_info &info)
{
	...
	int mousex, mousey;
	int mouse_flags = SDL_GetMouseState(0, &mousex,&mousey);
...
{
	blit_surface(location().x, location().y, surf_);
}
...
void filter_textbox::handle_text_changed(const wide_string& text) {
	const std::vector<shared_string> words = utils::split_shared(utils::wstring_to_string(text),' ');
	if (words == last_words)
		return;
	last_words = words;
	...
	for(size_t n = header_row_; n < items_to_filter_.size(); ++n) {
		std::vector<shared_string>::const_iterator w = words.begin();
		for(; w != words.end(); ++w)
		{
