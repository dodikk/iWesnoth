/* $Id: controller_base.cpp 35582 2009-05-11 23:49:01Z crab $ */
/*
	...
	case SDL_ACTIVEEVENT:
		if (event.active.type == SDL_APPMOUSEFOCUS && event.active.gain == 0) {
			if (get_mouse_handler_base().is_dragging()) {
				...
				int x, y;
				Uint8 mouse_flags = SDL_GetMouseState(0, &x, &y);
...
void controller_base::play_slice(bool is_delay_enabled)
{
	...
	int mousex, mousey;
	Uint8 mouse_flags = SDL_GetMouseState(0, &mousex, &mousey);
...
//		}
	...
	// KP: moved down here to draw buttons over everything
	//events::raise_draw_event();
	
	slice_end();
	
	// KP: moved at the end here, to make sure everything is drawn
	//get_display().flip();
...
void controller_base::show_menu(const std::vector<shared_string>& items_arg, int xloc, int yloc, bool context_menu)
{
