/* $Id: display.hpp 35509 2009-05-09 16:25:14Z silene $ */
/*
...
//	surface get_screen_surface() { return map_screenshot_ ? map_screenshot_surf_ : screen_.getSurface();}
	...
	void set_scroll_velocity(float xVelocity, float yVelocity);
...
protected:
	...
	std::vector<textureAtlasInfo> get_terrain_images(const map_location &loc,
	...
	// KP: added variables for scrolling that gradually slows down over time
...
public:
	...
	struct tblit{
		...
		tblit(const int x, const int y, const surface& surf,
				const SDL_Rect& clip = SDL_Rect(), textureRenderFlags flags = DRAW) :
			x(x),
			y(y),
			surf(1, surf),
			clip(clip),
		...
		// KP: added support for terrain atlas rendering
		tblit(const int x, const int y, const textureAtlasInfo& tinfo,
			  const SDL_Rect& clip = SDL_Rect(), textureRenderFlags flags = DRAW, unsigned long color = 0xFFFFFFFF, float brightness = 1.0f) :
		x(x),
		y(y),
		surf(),
		clip(clip),
		...
		tblit(const int x, const int y, const std::vector<textureAtlasInfo>& tinfo,
			  const SDL_Rect& clip = SDL_Rect(), textureRenderFlags flags = DRAW, unsigned long color = 0xFFFFFFFF, float brightness = 1.0f) :
		x(x),
		y(y),
		surf(),
		...
		// KP: added support for terrain atlas rendering
		std::vector<textureAtlasInfo> atlas;
		// KP: added support to draw reversed, to cut down required memory for unit images by 1/2
		textureRenderFlags drawFlags;	// see textureRenderFlags
		unsigned long drawColor;
	...
	enum tdrawing_layer{
		LAYER_TERRAIN_BG,          /**<
		                            ...
		                            * unit.
		                            */
		// KP: added the +10, so that more terain_bg can be piled on, eg transitions, tree edges
		LAYER_TERRAIN_TMP_BG = LAYER_TERRAIN_BG+10,      /**<
...
private:
	...
	// KP: added cache for get_terrain_images() function
	std::map<const map_location, std::vector<textureAtlasInfo> > get_terrain_cache_bg_, get_terrain_cache_fg_;
