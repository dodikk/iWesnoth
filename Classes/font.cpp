/* $Id: font.cpp 36234 2009-06-15 21:33:25Z soliton $ */
...
*/
	TTF_Font* ttfont = get_font(font_id(0, font_size_));
	font_style_setter const style_setter(ttfont, style_);
	TTF_SizeUTF8(ttfont, str_.c_str(), &w_, &h_);
}
...
public:
	...
	{
		// KP: maintain compatability with old style
		// lifetime was "frames at 20fps" -> ms
		if (lifetime == -1)
		{
			lifetime_ = 0;
			alpha_change_ = 0;
		...
		{
			lifetime_ = (lifetime*1000)/20;
			alpha_change_ = (255.0f*1000)/lifetime_;
		}
		age_ = 0;
		cur_alpha_ = 255.0f;
		// xmove/ymove was "pixels per frame at 20fps" -> pixels/s
		xmove_ = xmove*20;
		ymove_ = ymove*20;
	...
	bool expired() const { if (lifetime_ == 0) return false; return age_ >= lifetime_; }
...
//	update_rect(rect);
	...
	unsigned long timeDiffMs = (SDL_GetTicks() - last_update_);
	age_ += timeDiffMs;

	if (xmove_ != 0 || ymove_ != 0)
	{
		double xmov = xmove_*timeDiffMs/1000; 
		double ymov = ymove_*timeDiffMs/1000;
		move(xmov,ymov);
		if(age_ < lifetime_) 
		{
			if(alpha_change_ != 0 && (xmove_ != 0.0 || ymove_ != 0.0) && surf_ != NULL) 
			{
				...
				free_cache_texture(surf_);	// KP: clear texture cache
				double alphaDiff = (alpha_change_*timeDiffMs)/1000;
				cur_alpha_ -= alphaDiff;
				surf_.assign(adjust_surface_alpha(surf_,cur_alpha_,false));
			}
		}
	}
...
bool load_font_config()
{
	...
	std::string filename = get_cache_dir() + "/fonts";
	...
	if (!file_exists(checkFilename))
	{
		...
		cfg.saveCache(filename);
	}
	else
	{
		cfg.loadCache(filename);
	}
