/* $Id: game.cpp 35920 2009-05-28 18:53:05Z ai0867 $ */
/*
...
#define LOG_CONFIG std::cerr //LOG_STREAM(info, config)
...
bool game_controller::new_campaign()
{
	...
	// KP: changed to load on demand
//	config campaign_cfg;
//	cache_.get_config(game_config::path +"/data/campaigns/", campaign_cfg);
	
	// @TODO: do not forget to merge add-on campaigns
	
	config::child_list campaigns = game_config_.get_children("campaign");
	//config::child_list campaigns = campaign_cfg.get_children("campaign");
	...
	std::vector<shared_string> difficulty_options = utils::split_shared(difficulty_descriptions, ';');
	...
	// KP: added tutorial to campaign menu
	if (state_.campaign == "Gameplay_Tutorial")
	{
		set_tutorial();
	}
	else
...
#ifdef __IPHONEOS__
bool game_controller::play_multiplayer(int res)
#else	
...
sync_login *syncViewController;
	
void game_controller::sync_saves()
{
	
	syncViewController = [[sync_login alloc] initWithNibName:@"sync_login" bundle:nil];
	
	CGRect frameRect = syncViewController.view.frame;
#ifdef __IPAD__
...
#endif
	syncViewController.view.frame = frameRect;
	[gLandscapeView addSubview:syncViewController.view];
...
void game_controller::load_game_cfg(const bool force)
{
	...
	try {
		...
		// KP: avoid long loads at startup
		bool minimalLoad = false;
		if (game_config_.empty())
		{
			minimalLoad = true;
			cache_.get_config(game_config::path +"/data/minimal_launch.cfg", game_config_);
		}
		else
		{
			// normal full load
		...
		if (!minimalLoad)
		{
			...
			std::vector<shared_string> user_dirs;
			// Scan for standalone files
			std::vector<shared_string> user_files;
			...
			for(std::vector<shared_string>::const_iterator uc = user_files.begin(); uc != user_files.end(); ++uc) {
			...
			for(std::vector<shared_string>::const_iterator uc = user_dirs.begin(); uc != user_dirs.end(); ++uc){
...
//	gui2::init();
	...
	loadscreen::global_loadscreen->set_progress(10, _("Setting up GUI"));
	
	gui2::init();
	
	initTextureAtlas();
	initUnitTextureAtlas();
	...
	loadscreen::global_loadscreen->set_progress(40, _("Loading game configuration"));
...
//	}
	...
	loadscreen::global_loadscreen->set_progress(60, _("Searching for installed add-ons"));
	refresh_addon_version_info_cache();
	...
	loadscreen::global_loadscreen->set_progress(90, _("Loading title screen"));
	...
	for(;;){
		...
		if(gui2::new_widgets) {
			...
			while(res == gui::NOTHING) {
				...
				// kP: need to set dirty for next time...
				gui::set_background_dirty();
...
//#endif
	...
	try {
		...
		strncpy(buffer, argv[0], offset-1);
		buffer[offset-1] = 0;
...
//		exe_dir = "/Users/kyle/Desktop/wesnoth FREE/res";
		...
		strcat(buffer, "/data/_main.cfg");
				
		
		std::string testDir(buffer); //get_exe_dir();
		...
		// KP: pretend that we already started up, so we don't get a "failed to launch in time" error
		SDL_PumpEvents();
		SDL_Delay(20);
