/* $Id: game_display.cpp 35509 2009-05-09 16:25:14Z silene $ */
/*
...
{
	// KP: moved this here to refresh background before drawing
	if (invalidateUnit_ || invalidateGameStatus_)
		draw_all_panels();
...
void game_display::draw_minimap_units()
{
	...
	for(unit_map::const_iterator u = units_.begin(); u != units_.end(); ++u) {
		...
		SDL_SetRenderDrawColor(col.r, col.g, col.b, 0xff);
		SDL_RenderFill(&r);
	}
}
...
//	SDL_Rect top = {0,0,surf->w,bar_loc.y};
	...
	// KP: changed to just one filled surface, but it is not actually rendered... it is converted to openGL calls when it is detected
...
//	if(unfilled < height && alpha >= ftofxp(0.3)) 
	{
		...
		surface filled_surf = create_compatible_surface(bar_surf, bar_loc.w, height);
		//SDL_Rect filled_area = {0, 0, bar_loc.w, height-unfilled};
		SDL_Rect filled_area = {0, unfilled, bar_loc.w, height-unfilled};
		...
		drawing_buffer_add(LAYER_UNIT_BAR, loc, tblit(xpos + bar_loc.x, ypos + bar_loc.y, filled_surf));
	}
}
...
//	                         0,-2*turbo_speed(),lifetime,screen_area(),font::CENTER_ALIGN,NULL,0,font::ANCHOR_LABEL_MAP);
	// KP: change this to clip to map area
		font::add_floating_label(text,font::SIZE_XLARGE,color,get_location_x(loc)+zoom_/2,get_location_y(loc),
		                         0,-2*turbo_speed(),lifetime,map_area(),font::CENTER_ALIGN,NULL,0,font::ANCHOR_LABEL_MAP);	
}
		...
		bool bell)
{
	...
	// @TODO: chat doesn't have a filter, so app will need to be rated 17+... so for now, do not display chat messages :(	
	if (sender != "server")
		return;
