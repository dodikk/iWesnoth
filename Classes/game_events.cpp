/* $Id: game_events.cpp 34324 2009-03-31 03:06:37Z soliton $ */
/*
...
namespace game_events {
			...
			const vconfig cond, bool backwards_compat)
	{
		...
		{
			std::string cond_name(cond_i.get_key());
...
namespace {
	...
	WML_HANDLER_FUNCTION(allow_recruit,/*handler*/,/*event_info*/,cfg)
	{
		...
		const std::vector<shared_string>& types = utils::split_shared(type);
		const std::set<shared_string> recruits(types.begin(), types.end());
		(*teams)[index].add_recruits(recruits);
		for(std::vector<shared_string>::const_iterator i = types.begin(); i != types.end(); ++i) {
	...
	WML_HANDLER_FUNCTION(set_recruit,/*handler*/,/*event_info*/,cfg)
	{
		...
		std::vector<shared_string> recruit = utils::split_shared(cfg["recruit"]);
		if(recruit.size() == 1 && recruit.back() == "")
			recruit.clear();

		(*teams)[index].set_recruits(std::set<shared_string>(recruit.begin(), recruit.end()));
	...
	WML_HANDLER_FUNCTION(modify_side,/*handler*/,/*event_info*/,cfg)
	{
		...
		std::string team_name = sgettext(cfg["team_name"].c_str());
		std::string user_team_name = sgettext(cfg["user_team_name"].c_str());
		...
		if(team_index < teams->size()) {
			...
			if (!recruit_str.empty()) {
				std::vector<shared_string> recruit = utils::split_shared(recruit_str);
				if (recruit.size() == 1 && recruit.back() == "")
					recruit.clear();

				(*teams)[team_index].set_recruits(std::set<shared_string>(recruit.begin(),recruit.end()));
...
namespace {
	...
	WML_HANDLER_FUNCTION(set_variable,/*handler*/,/*event_info*/,cfg)
	{
		...
		shared_string& var = state_of_game->get_variable(name);
		...
		if(string_length_target.empty() == false) {
			const int value = string_length_target.length();
			var = str_cast(value);
		}
...
#ifdef __IPHONEOS__
  					rect.w/2,30,0.0,0.0,lifetime,rect,font::CENTER_ALIGN);
#else											
		...
		try {
			...
			if(loc.valid()) {
				...
				if(utils::string_bool(cfg["advance"], true) && get_replay_source().at_end()) {
					...
					const int side = lexical_cast_default<int>(side_str, -1);
	...
	WML_HANDLER_FUNCTION(endlevel,/*handler*/,/*event_info*/,cfg)
	{
		...
		const std::string result = cfg["result"]; //do not translate
