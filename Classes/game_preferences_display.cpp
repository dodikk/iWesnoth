/* $Id: game_preferences_display.cpp 33157 2009-02-28 16:12:49Z ivanovic $ */
/*
...
preferences_dialog::preferences_dialog(display& disp, const config& game_cfg)
	: gui::preview_pane(disp.video()),
	  ...
	  advanced_(disp.video(),std::vector<shared_string>(),false,-1,-1,NULL,&gui::menu::bluebg_style),
	  friends_(disp.video(),std::vector<shared_string>(),false,-1,-1,NULL,&gui::menu::bluebg_style),
...
#ifdef USE_TINY_GUI
	set_measurements(360, 230);		  // FIXME: should compute this, but using what data ?
#else
...
//	const int bottom_border = 0;
	...
	turbo_slider_label_.set_location(rect.x + horizontal_padding, ypos+5);
//	ypos += short_interline;
	SDL_Rect turbo_rect = { scroll_rect.x, ypos, scroll_rect.w, 0 };
...
//	hotkeys_button_.set_location(rect.x, bottom_row_y - hotkeys_button_.height());
	...
	ypos += item_interline; turbo_button_.set_location(rect.x, ypos);
	ypos += short_interline; 
	turbo_slider_label_.set_location(rect.x + horizontal_padding, ypos);
	ypos += short_interline;
...
void preferences_dialog::process_event()
{
	if (tab_ == GENERAL_TAB) {
		...
		if (autosavemax_slider_.value() == preferences::INFINITE_AUTO_SAVES)
			buf2 << _("Max Auto-Saves: ") << _("-");
		else
			buf2 << _("Max Auto-Saves: ") << autosavemax_slider_.value();
...
{
	std::vector<shared_string> advanced_items;
	const config::child_list& adv = game_cfg_.get_children("advanced_preference");
...
void preferences_dialog::set_friends_menu()
{
	...
	std::vector<shared_string> friends_items;
...
void preferences_dialog::set_selection(int index)
{
	...
	const bool hide_advanced = true; //tab_ != ADVANCED_TAB;
...
{
	int action = 0;
	std::vector<shared_string> options = disp.get_theme().get_known_themes();
	if(options.size()){
