/* $Id: halo.cpp 32970 2009-02-21 15:48:02Z alink $ */
/*
...
private:
	...
	//animated<shared_string> images_;
		...
		ORIENTATION orientation, bool infinite)
{
	...
	std::vector<shared_string> items = utils::split_shared(image);
	std::vector<shared_string>::const_iterator itor = items.begin();
	for(; itor != items.end(); ++itor) {
		const std::vector<shared_string>& items = utils::split_shared(*itor, ':');
		std::string str;
		int time;
