/* $Id: help.cpp 34664 2009-04-10 03:37:41Z dfranke $ */
/*
...
namespace {
	...
	config dummy_cfg;
	std::vector<shared_string> empty_string_vector;
	const int max_section_level = 15;
	const int menu_font_size = font::SIZE_NORMAL;
	const int title_size = font::SIZE_TITLE;
...
#ifdef __IPHONEOS__
	game_cfg = NULL;
	if (cfg->child_count("units") == 0)
	{
		unit_setup_needed_ = true;
	}
	else
	{
		unit_setup_needed_ = false;
	}
#else
...
std::vector<topic> generate_ability_topics(const bool sort_generated)
{
	...
	for(unit_type_data::unit_type_map::const_iterator i = unit_type_data::types().begin();
		...
		if (description_type(type) == FULL_DESCRIPTION) {
			...
			std::vector<shared_string> const* abil_vecs[2];
			abil_vecs[0] = &type.abilities();
			abil_vecs[1] = &type.adv_abilities();

			std::vector<shared_string> const* desc_vecs[2];
			desc_vecs[0] = &type.ability_tooltips();
			desc_vecs[1] = &type.adv_ability_tooltips();

			for(int i=0; i<2; ++i) {
				std::vector<shared_string> const& abil_vec = *abil_vecs[i];
				std::vector<shared_string> const& desc_vec = *desc_vecs[i];
...
*/
		...
		std::vector<shared_string> from_units = type_.advances_from();
		if (!from_units.empty())
		{
			ss << _("Advances from: ");
		...
		std::vector<shared_string> next_units = type_.advances_to();
		if (!next_units.empty()) {
			ss << _("Advances to: ");
		...
		if (race_it != unit_type_data::types().races().end()) {
			race_name = sgettext(race_it->second.plural_name().c_str());
		} else {
			race_name = _ ("Miscellaneous");
		}
		ss << _("Race: ");
		...
		if (!type_.abilities().empty()) {
			ss << _("Abilities: ");
			for(std::vector<shared_string>::const_iterator ability_it = type_.abilities().begin(),
		...
		if (!type_.adv_abilities().empty()) {
			...
			for(std::vector<shared_string>::const_iterator ability_it = type_.adv_abilities().begin(),
		...
		for(string_map::const_iterator dam_it = dam_tab.begin(), dam_end = dam_tab.end();
			...
			std::string lang_weapon = gettext(dam_it->first);
			push_tab_pair(row, lang_weapon);
...
void generate_races_sections(const config *help_cfg, section &sec, int level)
{
	...
	for(std::set<std::string>::iterator it = races.begin(); it != races.end(); it++) {
		...
		if (race_it != unit_type_data::types().races().end()) {
			title = sgettext(race_it->second.plural_name().c_str());
		} else {
...
UNIT_DESCRIPTION_TYPE description_type(const unit_type &type)
{
	...
	const std::set<shared_string> &encountered_units = preferences::encountered_units();
...
#ifdef __IPHONEOS__
			// Always open or close the section
			expanded(*sec) ? contract(*sec) : expand(*sec);
			update_visible_items(toplevel_);
			display_visible_items();
			
			// open the topic associated to this section
...
{
	std::vector<shared_string> menu_items;
...
void help_text_area::set_items()
{
	...
	for (it = parsed_items.begin(); it != parsed_items.end(); it++) {
		if (*it != "" && (*it)[0] == '[') {
			...
			try {
				config cfg;
				std::istringstream stream(std::string((*it).c_str()));
				read(cfg, stream);
...
//	surface const screen = video().getSurface();
	...
	for(std::list<item>::const_iterator it = items_.begin(), end = items_.end(); it != end; ++it) {
		...
		if (dst.y < static_cast<int>(loc.h) && dst.y + it->rect.h > 0) {
			...
			blit_surface(dst.x, dst.y, it->surf);
		}
	}
//	update_rect(loc);
}
...
//	surface const scr = screen.getSurface();
	...
	// KP: moved help cache loading to here
	
#ifdef __IPHONEOS__
	game_cfg = new config();
	
	std::string filename = get_cache_dir() + "/help";
	...
	if (!file_exists(checkFilename))
	{
		...
		scoped_istream stream = preprocess_file(get_wml_location("core/help.cfg"), &preproc);
		read(*game_cfg, *stream);
		
		config unitCfg;
		scoped_istream stream2 = preprocess_file(get_wml_location("core/units_help.cfg"), &preproc);
		read(unitCfg, *stream2);
		
		game_cfg->append(unitCfg);
		
		// save the cache
		
		game_cfg->saveCache(filename);
	}
	else
	{
		// load the cached version
		std::string filename = get_cache_dir() + "/help";
		game_cfg->loadCache(filename);
	}

	
	if (unit_setup_needed_)		// if loading a level, it already set up the units...
	{
		const config* const units = game_cfg->child("units");
		if(units != NULL) {
	...
	try {
		...
		for (;;) {
			...
			f.draw();
			hb.set_dirty(true);	// KP: added here
			events::raise_draw_event();
			if (key[SDLK_ESCAPE]) {
			...
			disp.flip();
			disp.invalidate_all();	// KP: added here
			disp.delay(5);
		}
	}
	catch (parse_error e) {
