/* $Id: hotkeys.cpp 34027 2009-03-22 12:41:54Z ilor $ */
/*
...
void load_descriptions()
{
	for (size_t i = 0; hotkey_list_[i].command; ++i) {
		...
		hotkeys_[i].set_description(hotkey_list_[i].description);
	}
}
...
void command_executor::show_menu(const std::vector<shared_string>& items_arg, int xloc, int yloc, bool context_menu, display& gui)
{
	...
	if (can_execute_command(hotkey::get_hotkey(items.front()).get_id(), 0)){
		...
		std::vector<shared_string> menu = get_menu_images(items);
...
std::vector<shared_string> command_executor::get_menu_images(const std::vector<shared_string>& items){
	std::vector<shared_string> result;
	bool has_image = false;

	for(size_t i = 0; i < items.size(); ++i) {
		shared_string const& item = items[i];
	...
	if(has_image)
		for(std::vector<shared_string>::iterator i = result.begin(); i != result.end(); ++i)
			if(*(i->begin()) != IMAGE_PREFIX)
				//i->insert(i->begin(), COLUMN_SEPARATOR);
				// TODO: (KP) verify that this is correct
				*i = COLUMN_SEPARATOR + *i;
	return result;
}
