/* $Id: image.cpp 35477 2009-05-08 00:06:52Z alink $ */
/*
...
//typedef boost::unordered_map<image::locator::value, int> locator_finder_t;
...
std::set<shared_string> precached_dirs;
...
std::vector<shared_string> team_colors;
...
void flush_cache()
{
	...
	// KP: don't waste memory on this
	brightened_images_.set_max_size(5);
	semi_brightened_images_.set_max_size(5);
}
...
locator::value::value(const shared_string& filename) :
	type_(FILE), filename_(filename),  loc_(), modifications_(),
	center_x_(0), center_y_(0)
...
{
	const surface mother_surface(get_image(val_.filename_.c_str(), UNSCALED));
...
void set_team_colors(const std::vector<shared_string>* colors)
{
	if (colors == NULL)
		team_colors.clear();
	else {
...
surface get_image(const image::locator& i_locator, TYPE type, bool cacheIt)
{
	surface res(NULL);
	image_cache *imap;
...
static void precache_file_existence_internal(const std::string& dir, const std::string& subdir)
{
	...
	std::vector<shared_string> files_found;
	std::vector<shared_string> dirs_found;
	get_files_in_dir(checked_dir, &files_found, &dirs_found,
			FILE_NAME_ONLY, NO_FILTER, DONT_REORDER);

	for(std::vector<shared_string>::const_iterator f = files_found.begin();
			f != files_found.end(); ++f) {
		image_existence_map[subdir + *f] = true;
	}

	for(std::vector<shared_string>::const_iterator d = dirs_found.begin();
			d != dirs_found.end(); ++d) {
...
//	while(cache_size_ > cache_max_size_-100) {
	while(cache_size_ > cache_max_size_) {
