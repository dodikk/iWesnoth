/* $Id: image.hpp 35477 2009-05-08 00:06:52Z alink $ */
/*
...
//		enum type { NONE, FILE, SUB_FILE };
	...
	public:
		...
		struct value {
			...
			shared_string filename_;
			map_location loc_;
			//std::string modifications_;
			shared_string modifications_;
			int center_x_;
			int center_y_;
		...
		public:
			...
			{
				fwrite(&type_, sizeof(type_), 1, file);
				cacheSaveString(file, filename_);
				loc_.saveCache(file);
				cacheSaveString(file, modifications_);
				fwrite(&center_x_, sizeof(center_x_), 1, file);
				fwrite(&center_y_, sizeof(center_y_), 1, file);
			}
			...
			void loadCache(MEMFILE *file, char *loadBuffer)
			{
				...
				mread(&type_, sizeof(type_), 1, file);
				//filename_ = cacheLoadstd::string(file);
				cacheLoadString(file, &str, &strSize);
				filename_.assign(str, strSize);
				loc_.loadCache(file, loadBuffer);
				//modifications_ = cacheLoadstd::string(file);
				cacheLoadString(file, &str, &strSize);
				modifications_.assign(str, strSize);
				mread(&center_x_, sizeof(center_x_), 1, file);
				mread(&center_y_, sizeof(center_y_), 1, file);
			}
		...
		{ if(index_ != -1 ) cache.get_element(index_) = cache_item<surface>(image); cache.cache_size_++; cache.on_load(index_); }
	...
	private:
		...
		int index_;
	public:		// KP: bend the rules a bit to make life easier
		value val_;
	...
	public:
		void saveCache(::FILE *file) const
		{
			fwrite(&index_, sizeof(index_), 1, file);
			val_.saveCache(file);
		}
		
		void loadCache(MEMFILE *file, char *loadBuffer)
		{
			mread(&index_, sizeof(index_), 1, file);
			val_.loadCache(file, loadBuffer);
			init_index();
		}
	};
	...
	void set_team_colors(const std::vector<shared_string>* colors = NULL);
	...
	///SDL_FreeSurface()
	surface get_image(const locator& i_locator, TYPE type=UNSCALED, bool cacheIt=true);
