/* $Id: intro.cpp 32970 2009-02-21 15:48:02Z alink $ */
/*
...
void show_intro(display &disp, const vconfig& data, const config& level)
{
	...
	for(vconfig::all_children_iterator i = data.ordered_begin();
		...
		std::pair<std::string, const vconfig> item = *i;
...
//			0,0,0,1.0,video.getSurface());
	SDL_Rect fillRect = {0, 0, video.getx(), video.gety()};
	SDL_SetRenderDrawColor(0, 0, 0, 0xff);
	SDL_RenderFill(&fillRect);
	...
	if(background_name.empty() == false) {
		...
		background.assign(image::get_image(background_name, image::UNSCALED, false));
	}
...
#ifdef USE_TINY_GUI
	// KP: shift the image up a bit, based on what is covered by the text
	if(images.empty())
		dstrect.y -= 75/2;
#endif
...
//	SDL_BlitSurface(background,NULL,video.getSurface(),&dstrect);
	blit_surface(dstrect.x, dstrect.y, background);

#ifdef USE_TINY_GUI
	textx = 10;
	int xbuttons = video.getx() - 35 - 50;
	int ybuttons = video.gety() - 35; //dstrect.y + dstrect.h - 20;
	...
	if(show_title) {
		...
		const SDL_Rect tintRect = {dstrect.x + 15,dstrect.y + 15,txt_shadow_rect.w + 10,txt_shadow_rect.h + 10};
		...
		font::draw_text(&video,area,font::SIZE_XLARGE,font::BIGMAP_COLOUR,scenario,
						dstrect.x + 20,dstrect.y + 20);
	}
	...
	if(!images.empty()) {
		...
		for(vconfig::child_list::const_iterator i = images.begin(); i != images.end(); ++i){
			...
			blit_surface(image_rect.x, image_rect.y, img);
			...
			if(pass == false) {
				for(int i = 0; i != 50; ++i) {
					...
					int a, b;
					const int mouse_flags = SDL_GetMouseState(0, &a,&b);
...
//	video.blit_surface(0, y, blur);
	...
	// KP: hmmmmm... how to blur in openGL? just fade instead?
	SDL_SetRenderDrawColor(0, 0, 0, 0x80);
	SDL_RenderFill(&blur_rect);
