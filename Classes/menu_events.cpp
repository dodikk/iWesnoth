/* $Id: menu_events.cpp 36222 2009-06-15 12:27:45Z soliton $ */
/*
...
private:
	...
	void menu_handler::recruit(const bool browse, const unsigned int team_num, const map_location& last_hex)
	{
		...
		for(std::set<shared_string>::const_iterator it = recruits.begin(); it != recruits.end(); ++it) {
...
//				gui::dialog::BUTTON_HELP);
	...
	void menu_handler::do_recruit(const std::string& name, const int unsigned team_num, const map_location& last_hex)
	{
		...
		int recruit_num = 0;
		const std::set<shared_string>& recruits = current_team.recruits();
		for(std::set<shared_string>::const_iterator r = recruits.begin(); ; ++r) {
			if (r == recruits.end()) {
				return;
			}
	...
	void menu_handler::recall(const unsigned int team_num, const map_location& last_hex)
	{
		...
		if(recall_list.empty()) {
			...
			std::vector<shared_string> options, options_to_filter;
	...
	{
		std::vector<shared_string> options;
...
/*				register_command("buff", &console_handler::do_buff,
	 ...
	 *
	 * @param data               std::string of the form: '<level> <domain>'
	 */
