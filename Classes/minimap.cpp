/* $Id: minimap.cpp 32647 2009-02-07 15:31:18Z ilor $ */
/*
		...
		for(int x = 0; x != map.total_width(); ++x) {
			...
			if(map.on_board(loc)) {
				...
				if(i == cache->end()) {
					...
					// KP: do NOT cache it, just the mini image is cached...
					surface tile(get_image("terrain/" + map.get_terrain_info(terrain).minimap_image() + ".png",image::HEXED, false));
					...
					if(map.get_terrain_info(terrain).is_combined()) {
						...
						// KP: do NOT cache it
						surface overlay(get_image("terrain/" + map.get_terrain_info(terrain).minimap_image_overlay() + ".png", image::HEXED, false));
