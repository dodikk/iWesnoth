/* $Id: mouse_events.cpp 36315 2009-06-21 10:47:04Z silene $ */
/*
...
void mouse_handler::left_mouse_up(int x, int y, const bool browse)	
{
	...
	// center on the clicked tile
	//gui().scroll_to_tile(hex, display::SCROLL, false);
	
	// clicking on a friendly unit selects it
	unit_map::iterator uTest = find_unit(hex);
	if (uTest != units_.end())
	{
		if (uTest->second.side() == team_num_)
		{
	...
	// clicking to a different tile is just like a mouse hover, ignore the click
	if (last_hex_ != last_clicked_hex_)
	{
		last_clicked_hex_ = last_hex_;
		return;
	}
...
void mouse_handler::select_hex(const map_location& hex, const bool browse) {
	...
	if(hex.valid() && u != units_.end() && !u->second.get_hidden()) {
...

		{
			...
			bool teleport = u->second.get_ability_bool("teleport", u->first);
