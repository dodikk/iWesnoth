/* $Id: multiplayer_connect.cpp 35922 2009-05-28 22:41:27Z soliton $ */
/*
...
connect::side::side(connect& parent, const config& cfg, int index) :
	...
	combo_ai_algorithm_(parent.disp(), std::vector<shared_string>()),
	combo_faction_(parent.disp(), std::vector<shared_string>()),
	combo_leader_(parent.disp(), std::vector<shared_string>()),
	combo_gender_(parent.disp(), std::vector<shared_string>()),
	...
	label_gold_(parent.video(), "Gold: 100", font::SIZE_SMALL, font::LOBBY_COLOUR),
	label_income_(parent.video(), _("Income: Normal"), font::SIZE_SMALL, font::LOBBY_COLOUR),
	...
	llm_(parent.era_sides_, enabled_ ? &combo_leader_ : NULL, enabled_ ? &combo_gender_ : NULL)
{
	...
	if (!enabled_) {
		faction_ = 0;
		std::vector<shared_string> pseudo_factions;
		...
		std::vector<shared_string> gender_name_pseudolist;
		...
		if(!leader_.empty()) {
			...
			std::vector<shared_string> leader_name_pseudolist;
			unit_type_data::unit_type_map::const_iterator leader_name = unit_type_data::types().find_unit_type(leader_);
			if(leader_name == unit_type_data::types().end()) {
				leader_name_pseudolist.push_back("?");
			} else {
				leader_name_pseudolist.push_back(leader_name->second.type_name());
		...
		if(faction_ == 0) {
			...
			if(!cfg_.get_attribute("faction").empty()) {
				...
				find = utils::split_shared(cfg_["recruit"]);
			...
			while(faction != parent.era_sides_.end()) {
				...
				std::vector<shared_string> recruit;
				recruit = utils::split_shared(side[search_field]);
				std::vector<shared_string>::const_iterator search = find.begin();
...
void connect::side::add_widgets_to_scrollpane(gui::scrollpane& pane, int pos)
{
	...
	pane.add_widget(&combo_leader_,  curX, 35 + pos);
	curX += 100;
	pane.add_widget(&combo_gender_,  curX, 35 + pos);
	pane.add_widget(&combo_team_,    curX, 5 + pos);
	curX += 100;
	pane.add_widget(&combo_colour_,  curX, 5 + pos);
	curX = 22 + 100;
	pane.add_widget(&slider_gold_,   curX, 65 + pos);
	pane.add_widget(&label_gold_,    curX, 95 + pos);
	curX += 100;
	pane.add_widget(&slider_income_, curX,  65 + pos);
	pane.add_widget(&label_income_,  curX, 95 + pos);
}
...
void connect::side::process_event()
{
	...
	if (slider_gold_.value() != gold_) {
		...
		label_gold_.set_text("Gold: " + lexical_cast_default<std::string>(gold_, "0"));
		changed_ = true;
	}
	if (slider_income_.value() != income_) {
		...
		label_income_.set_text("Income: " + buf.str());
		changed_ = true;
	}
}
...
void connect::side::init_ai_algorithm_combo()
{
	...
	int sel = 0;
	std::vector<shared_string> ais = parent_->ai_algorithms_;
...
{
	std::vector<shared_string> factions;
	...
	if (id_.empty()) {
		...
		case CNTR_LOCAL:
			if(enabled_ && cfg_.get_attribute("save_id").empty()) {
				res["save_id"] = preferences::login() + res["side"];
			}
			res["id"] = preferences::login() + res["side"];
...
void connect::side::update_user_list()
{
	...
	typedef std::vector<shared_string> name_list;
...
void connect::side::resolve_random()
{
	...
	if (llm_.get_gender() == "random" || solved_random_leader) {
		unit_type_data::unit_type_map::const_iterator ut = unit_type_data::types().find_unit_type(leader_.empty() ? llm_.get_leader() : leader_);
...
void connect::layout_children(const SDL_Rect& rect)
{
	...
	int curX = left + 8;
	waiting_label_.set_location(curX, bottom-left_button->height() + 4);
	curX += 22;
	type_title_label_.set_location(curX, top+35);
	curX += 100;
	faction_title_label_.set_location(curX, top+35);
	curX += 100;
	team_title_label_.set_location(curX, top+35);
	curX += 100;
	colour_title_label_.set_location(curX, top+35);
	curX += 6666;
	gold_title_label_.set_location(curX, top+35);
	curX += 6666;
	income_title_label_.set_location(curX, top+35);
...
void connect::lists_init()
{
	...
	if(params_.use_map_settings) {
		for(sd = sides.first; sd != sides.second; ++sd) {
			...
			t_string team_name = (**sd)["team_name"];
			//shared_string& team_name = (**sd)["team_name"];
			//t_string& user_team_name = (**sd)["user_team_name"];
			t_string user_team_name = (**sd)["user_team_name"];
			//shared_string& user_team_name = (**sd)["user_team_name"];

			if(team_name.empty())
			{
				team_name = lexical_cast<std::string>(side_num);
				(**sd)["team_name"] = team_name;
			}

			if(user_team_name.empty())
			{
				user_team_name = team_name;
				(**sd)["user_team_name"] = user_team_name;
			}

			std::vector<shared_string>::const_iterator itor = std::find(team_names_.begin(),
															team_names_.end(), team_name);
			if(itor == team_names_.end()) {
				team_names_.push_back(sgettext(team_name.c_str()));
				//user_team_names_.push_back(user_team_name.to_serialized());
				user_team_names_.push_back(sgettext(user_team_name.c_str()));
				//player_teams_.push_back(user_team_name.str());
				player_teams_.push_back(sgettext(user_team_name.c_str()));
			}
		}
	} else {
...
void connect::update_playerlist_state(bool silent)
{
	...
	if (gamelist().child("user") != NULL) {
		...
		std::vector<shared_string> playerlist;
