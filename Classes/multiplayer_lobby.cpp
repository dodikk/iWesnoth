/* $Id: multiplayer_lobby.cpp 33741 2009-03-17 03:35:09Z soliton $ */
/*
...
void gamebrowser::draw_row(const size_t index, const SDL_Rect& item_rect, ROW_TYPE /*type*/) {
	...
	if (game.mini_map != NULL) {
		...
		blit_surface(minimap_x, minimap_y, game.mini_map);
	}
	...
	const surface name_surf(font::get_rendered_text(
		...
		blit_surface(xpos, ypos, name_surf);

	// Draw status text
	if(status_text) {
		// Align the bottom of the text with the game name
			blit_surface(item_rect.x + item_rect.w - margin_ - status_text_width,
	...
	if(map_info_surf) {
		blit_surface(xpos, ypos - map_info_surf->h/2, map_info_surf);
	}
	...
	if(observer_icon) {
		blit_surface(xpos, ypos - observer_icon->h, observer_icon);
	...
	if(gold_icon) {
		blit_surface(xpos, ypos - gold_icon->h/2, gold_icon);
	...
	if(gold_text) {
		blit_surface(xpos, ypos - gold_text->h/2, gold_text);
	...
	if(xp_icon) {
		blit_surface(xpos, ypos - xp_icon->h/2, xp_icon);
	...
	const surface xp_text(font::get_rendered_text(game.xp, font::SIZE_NORMAL,
	    font::NORMAL_COLOUR));
	if(xp_text) {
		blit_surface(xpos, ypos - xp_text->h/2, xp_text);
	...
	if(!game.time_limit.empty()) {
		...
		blit_surface(xpos, ypos - time_icon->h/2, time_icon);
		...
		const surface time_text(font::get_rendered_text(game.time_limit,
		    font::SIZE_NORMAL, font::NORMAL_COLOUR));
		blit_surface(xpos, ypos - time_text->h/2, time_text);
	...
	if(vision_icon) {
		blit_surface(xpos, ypos - vision_icon->h/2, vision_icon);
	...
	const surface vision_text(font::get_rendered_text(
	    ...
	    game.use_map_settings ? font::GRAY_COLOUR : font::NORMAL_COLOUR));
	if(vision_text) {
		blit_surface(xpos, ypos - vision_text->h/2, vision_text);
	}
	...
	if (game.use_map_settings) {
		...
		blit_surface(xpos, ypos - map_settings_text->h/2, map_settings_text);
	}
}
			...
			if(level_cfg) {
				...
				if (map_hashes_ && !games_.back().reloaded) {
					...
					for(string_map::const_iterator i = map_hashes_->values.begin(); i != map_hashes_->values.end(); ++i) {
						//if(i->first == (**game)[keyIndex] && i->second == hash) {
						if(!strcmp(i->first.c_str(), (**game)["mp_scenario"].c_str()) && i->second == hash) {
							hash_found = true;
							break;
						}
					}
