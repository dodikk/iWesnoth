/* $Id: multiplayer_ui.cpp 35301 2009-04-28 17:56:25Z mordante $ */
/*
...
ui::ui(game_display& disp, const std::string& title, const config& cfg, chat& c, config& gamelist) :
	...
	users_menu_(disp.video(), std::vector<shared_string>(), false, -1, -1, NULL, &umenu_style),
	...
	lobby_clock_(0)
{
	background_.assign(image::get_image("misc/lobby_iphone.png", image::UNSCALED, false));
...
//	surface background(image::get_image("misc/lobby_iphone.png"));
//
//	if(background == NULL)
//		return;
...
{
	title_.hide(hide);
	chat_textbox_.hide(true);
#ifndef USE_TINY_GUI
	entry_textbox_.hide(true);
#endif
	users_menu_.hide(true);
}
...
void ui::layout_children(const SDL_Rect& /*rect*/)
{
	...
	chat_textbox_.set_location(xscale(11) + 4, 320);
...
void ui::gamelist_updated(bool silent)
{
	...
	std::vector<shared_string> user_strings;
	std::vector<shared_string> menu_strings;
...
void ui::set_user_menu_items(const std::vector<shared_string>& list)
{
	users_menu_.set_items(list,true,true);

	// Try to keep selected player
	std::vector<shared_string>::const_iterator i =
...
void ui::set_user_list(const std::vector<shared_string>& list, bool silent)
{
	if(!silent) {
