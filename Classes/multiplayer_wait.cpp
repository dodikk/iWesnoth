/* $Id: multiplayer_wait.cpp 36137 2009-06-11 12:48:09Z ivanovic $ */
/*
...
wait::leader_preview_pane::leader_preview_pane(game_display& disp,
	...
	color_(color),
	leader_combo_(disp, std::vector<shared_string>()),
	gender_combo_(disp, std::vector<shared_string>()),
...
//	surface const screen = video().getSurface();
	...
	if(selection_ < side_list_.size()) {
		...
		if(!unit_image.null()) {
			...
			blit_surface(image_rect.x, image_rect.y, unit_image);
		}
...
wait::wait(game_display& disp, const config& cfg,
	...
	game_menu_(disp.video(), std::vector<shared_string>(), false, -1, -1, NULL, &gui::menu::bluebg_style),
	level_(),
	state_(),
	stop_updates_(false)
{
...
void wait::join_game(bool observe)
{
	...
	if (!observe) {
		...
		if(allow_changes) {
			...
			std::vector<shared_string> choices;
...
void wait::generate_menu()
{
	...
	std::vector<shared_string> details;
	...
	for(config::child_list::const_iterator s = sides.begin(); s != sides.end(); ++s) {
		...
		if (!leader_image.empty()) {
			...
			if(side_name[0] == font::IMAGE) {
				...
				side_name = IMAGE_PREFIX + leader_image + COLUMN_SEPARATOR + side_name;
			}
		}
		...
		//str	<< COLUMN_SEPARATOR << t_string().from_serialized(sd["user_team_name"]);
		str	<< COLUMN_SEPARATOR << sgettext(sd["user_team_name"].c_str());
