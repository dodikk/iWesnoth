/* $Id: sdl_utils.cpp 34753 2009-04-12 06:01:26Z boucman $ */
/*
...
static SDL_PixelFormat& get_neutral_pixel_format()
	{
		...
		if(first_time) {
			...
			// KP: changed to match iPhone ABGR8888
			//surface surf(SDL_CreateRGBSurface(SDL_SWSURFACE,1,1,32,0xFF,0xFF00,0xFF0000,0xFF000000));
			// but then other functions here, and all color constants, need to be changed...
			format = *surf->format;
			format.palette = NULL;
		}
...
surface create_optimized_surface(surface const &surf)
{
	...
	SDL_PixelFormat pf = get_neutral_pixel_format();
	surface const result = SDL_ConvertSurface(surf, &pf, 0);
...
// @TODO: KP: this function is pretty inefficient... try SDL optimized blitters
...
void draw_rectangle(int x, int y, int w, int h, Uint32 colour/*,surface target*/)
{
	...
	SDL_SetRenderDrawColor((colour >> 16) & 0xff, (colour >> 8) & 0xff, colour & 0xff, 0xff);
	SDL_RenderFill(&top);
	SDL_RenderFill(&bot);
	SDL_RenderFill(&left);
	SDL_RenderFill(&right);
	 */
	fill_rect(colour, &top);
	fill_rect(colour, &bot);
	fill_rect(colour, &left);
	fill_rect(colour, &right);
}
...
//	Uint32 col = SDL_MapRGBA(target->format, color.r, color.g, color.b, color.unused);
	...
	SDL_SetRenderDrawColor(color.r, color.g, color.b, 0xff);
	SDL_RenderFill(&r);
...
SDL_Rect gClip;
void setClipRect(const SDL_Rect *clip)
{
	gClip = *clip;
}
void getClipRect(SDL_Rect *clip)
{
	*clip = gClip;
}
