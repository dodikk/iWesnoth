/* $Id: parser.cpp 35988 2009-06-01 11:41:05Z silene $ */
/*
...
std::string leading_spaces_buffer;
			          ...
			          std::string const &error_string)
{
	...
	boost::algorithm::replace_all(result, std::string("$") + var.first, std::string(var.second.c_str()));
...
void write_key_val(std::ostream &out, const std::string &key, const std::string &value, unsigned int level, std::string& textdomain)
{
//	bool first = true;
	if (value.empty()) {
		return;
...
static void write_internal(config const &cfg, std::ostream &out, std::string& textdomain, size_t tab = 0)
{
	...
	for(config::all_children_iterator j = cfg.ordered_begin(), j_end = cfg.ordered_end(); j != j_end; ++j) {
		const config::all_children_iterator::value_type& item = *j;
