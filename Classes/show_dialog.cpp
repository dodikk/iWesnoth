/* $Id: show_dialog.cpp 34873 2009-04-13 15:17:37Z silene $ */
/*
...
// KP: global variable that signals we need a full redraw
extern bool gRedraw;
...
int dialog_frame::top_padding() const {
	...
	if(!title_.empty()) {
		padding += font::get_max_height(font::SIZE_TITLE) + 2*dialog_frame::title_border_h + 3;
	}
	return padding;
}
...
SDL_Rect dialog_frame::draw_title(CVideo* video)
{
	...
	rect = screen_area();
	return font::draw_text(video, rect, font::SIZE_TITLE, font::TITLE_COLOUR,
	                       title_, dim_.title.x, dim_.title.y-4, false, TTF_STYLE_BOLD);
}
...
#endif
		for(std::vector<button*>::const_iterator b = buttons_->begin(); b != buttons_->end(); ++b) {
			(**b).set_location(buttons_area.x, buttons_area.y+3);
			buttons_area.x += (**b).width() + ButtonHPadding;
		}
	}

	if(help_button_ != NULL) {
		help_button_->set_location(dim_.interior.x+ButtonHPadding, buttons_area.y+3);
	}
}
