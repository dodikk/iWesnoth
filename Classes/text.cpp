/* $Id: text.cpp 35983 2009-05-31 19:55:13Z mordante $ */
/*
...
//		pango_layout_get_pixel_extents(layout_, NULL, &rect_);
		...
		else
		{
			...
			const std::vector<std::string> lines = utils::split(wrapped_text, '\n', utils::STRIP_SPACES);
			SDL_Rect txt_area;
			txt_area.w = txt_area.h = 0;
			...
			for(std::vector<std::string>::const_iterator line = lines.begin(); line != lines.end(); ++line) 
			{
				...
				SDL_Rect lineRect = line_size(*line, font_size_);
				txt_area.w = std::max<int>(lineRect.w, txt_area.w);
				txt_area.h += lineRect.h;
			}
			rect_.width = txt_area.w;
			rect_.height = txt_area.h;
		}
	}
}
...
*/ 
	...
	// KP: for iPhone, forget pango and just use SDL_ttf
	if(surface_dirty_ || force) 
	{
		recalculate(force);
		surface_dirty_ = false;
		...
		SDL_Color clr;
		clr.r = foreground_colour_ >> 24;
		clr.g = (foreground_colour_ >> 16) & 0xFF;
		...
		if (maximum_width_ < 0)
		{
			...
			rect_.width = rect.w;
			rect_.height = rect.h;
			surface_ = get_rendered_text(text_, font_size_, clr);
		}
		else
		{
			// KP: code from elsewhere
			...
			const std::vector<std::string> lines = utils::split(wrapped_text, '\n', utils::STRIP_SPACES);
			SDL_Rect txt_area;
			txt_area.y = txt_area.x = txt_area.w = txt_area.h = 0;
			
			//surface const screen(SDL_GetVideoSurface());
			surface_ = create_neutral_surface(rect_.width, rect_.height); // create_compatible_surface(screen, rect_.width, rect_.height);
			...
			for(std::vector<std::string>::const_iterator line = lines.begin(); line != lines.end(); ++line) 
			{
				txt_area = font::draw_text_s(surface_,area,font_size_,clr,*line,txt_area.x,txt_area.y);
				txt_area.y += txt_area.h;
			}
		}
	
		//surface_ = get_rendered_text(text_, 8, clr);
	}
}
