/* $Id: unit_types.hpp 32339 2009-01-27 20:56:56Z shadowmaster $ */
/*
...
typedef std::map<shared_string,unit_movement_type> movement_type_map;
...
public:
	...
	const shared_string& undead_variation() const { return undead_variation_; }
	...
	const shared_string& usage() const { return usage_; }
	const shared_string& image() const { return image_; }
	const shared_string& image_profile() const;
	...
	std::vector<shared_string> advances_to() const { return advances_to_; }
	std::vector<shared_string> advances_from() const { return advances_from_; }
	...
	const std::vector<shared_string>& abilities() const { return abilities_; }
	const std::vector<shared_string>& ability_tooltips() const { return ability_tooltips_; }

	// some extra abilities may be gained through AMLA advancements
	const std::vector<shared_string>& adv_abilities() const { return adv_abilities_; }
	const std::vector<shared_string>& adv_ability_tooltips() const { return adv_ability_tooltips_; }
	...
	std::vector<shared_string> get_ability_list() const;
	...
	const shared_string& race() const;
...
private:
    ...
    int max_attacks_;
    int cost_;
    shared_string usage_;
    shared_string undead_variation_;

    shared_string image_;
    shared_string image_profile_;
	...
	typedef std::map<shared_string,unit_type*> variations_map;
	...
	std::vector<shared_string> abilities_, adv_abilities_;
	std::vector<shared_string> ability_tooltips_, adv_ability_tooltips_;

	bool zoc_, hide_help_;

	std::vector<shared_string> advances_to_;
	std::vector<shared_string> advances_from_;
