/* $Id: button.cpp 31858 2009-01-01 10:27:41Z mordante $ */
/*
...
#ifdef __IPHONEOS__
// KP: just rescaled the image...
//	base_width_ = (button_image->w * 3) / 4;
#endif
...
void button::draw_contents()
{
	...
	if (!enabled()) {
		// KP: added inactiveImage_ cache
		if (inactiveImage_ == NULL)
		{
			static const Uint32 disabled_btn_color = 0xAAAAAA;
			static const double disabled_btn_adjust = 0.18;
			inactiveImage_ = blend_surface(greyscale_image(image), disabled_btn_adjust, disabled_btn_color);
		}
		image = inactiveImage_;
		button_colour = font::GRAY_COLOUR;
	}

	blit_surface(loc.x, loc.y, image);
	if (type_ != TYPE_IMAGE){
		clipArea.x += offset;
		clipArea.y += offset;
		clipArea.w -= 2*offset;
