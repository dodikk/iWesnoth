/* $Id: menu.cpp 32635 2009-02-07 13:00:40Z baufo $ */
/*
...
menu::menu(CVideo& video, const std::vector<shared_string>& items,
...
void menu::fill_items(const std::vector<shared_string>& items, bool strip_spaces)
{
	for(std::vector<shared_string>::const_iterator itor = items.begin();
	    itor != items.end(); ++itor) {
...
void menu::set_items(const std::vector<shared_string>& items, bool strip_spaces, bool keep_viewport)
{
