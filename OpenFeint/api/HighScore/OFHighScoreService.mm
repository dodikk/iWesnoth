...
+ (void) getPage:(NSInteger)pageIndex pageSize:(NSInteger)pageSize forLeaderboard:(NSString*)leaderboardId comparedToUserId:(NSString*)comparedToUserId friendsOnly:(BOOL)friendsOnly silently:(BOOL)silently onSuccess:(const OFDelegate&)onSuccess onFailure:(const OFDelegate&)onFailure
{
...
{
	[OFHighScoreService setHighScore:score withDisplayText:displayText withCustomData:nil forLeaderboard:leaderboardId silently:NO onSuccess:onSuccess onFailure:onFailure];
}
...
+ (void) setHighScore:(int64_t)score withDisplayText:(NSString*)displayText withCustomData:(NSString*)customData forLeaderboard:(NSString*)leaderboardId silently:(BOOL)silently onSuccess:(const OFDelegate&)onSuccess onFailure:(const OFDelegate&)onFailure
{
    ...
    NSString* lastLoggedInUser = [OpenFeint lastLoggedInUserId];
	[OFHighScoreService localSetHighScore:score forLeaderboard:leaderboardId forUser:lastLoggedInUser displayText:displayText customData:customData serverDate:nil addToExisting:NO];
	...
	if (!silently)
	{
		OFNotificationData* notice = [OFNotificationData dataWithText:([OpenFeint isOnline] ? @"Submitted High Score" : @"Saved High Score Locally") andCategory:kNotificationCategoryHighScore andType:kNotificationTypeSuccess];
